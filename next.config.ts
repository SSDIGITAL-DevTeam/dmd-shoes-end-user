...
const resolveEnv = () => {
  ...
  if (!apiUrl)
    throw new Error(
      "Missing NEXT_PUBLIC_API_URL (fallbacks: VITE_API_BASE_URL). Please add it to .env.local.",
    );
  if (!assetUrl)
    throw new Error(
      "Missing NEXT_PUBLIC_ASSET_URL (fallbacks: VITE_ASSET_BASE_URL). Please add it to .env.local.",
    );
  return {
