...
import ProductPreviewClick from "./ProductPreviewClick";
...
export default function ProductPC6601() {
  const [productPreviewSelection, setProductPreviewSelection] =
    useState<ProductPreviewType>(productPreviews[0]);
  return (
    <div>
      <Container className="py-[40px]">
        ...
        <div className="flex flex-col md:flex-row gap-8">
              ...
              {productPreviews.map((preview, index) => (
                <button
                  ...
                  style={{
                    ...
                    opacity:
                      productPreviewSelection.id === preview.id ? 1 : 0.2,
                  }}
                  aria-label={`Preview ${index + 1}`} // ✅ aksesibilitas
                />
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 space-y-[20px]">
            ...
            <p className={`${plusJakartaSans.className} text-sm text-black`}>
              Terbuat dari bahan plastik bening berkualitas, hak ini memberikan
              tampilan minimalis dan elegan serta trans paran yang estetik.
            </p>
            <p
              className={`${plusJakartaSans.className} text-[#D97706] text-sm`}
            >
              Harga Belum Termasuk Ongkir
            </p>
            <p className="text-3xl font-bold ">
              Rp 15.000{" "}
              <span className="text-sm text-gray-500">Per pasang</span>
            </p>
            ...
            <div className="flex flex-col lg:flex-row gap-3 mt-6">
              <div className="flex w-full lg:w-auto gap-3 ">
                <Link
                  ...
                  className="w-1/2 lg:w-auto flex justify-center items-center gap-2 bg-green-500 text-white px-4 py-2 rounded-[8px]"
                >
                ...
                <Link
                  ...
                  className="w-1/2 lg:w-auto flex justify-center items-center gap-2 bg-primary text-white px-4 py-2 rounded-[8px]"
                >
                  <CiMail size={28} aria-hidden="true" />
                  <span className="sr-only">Kirim Email</span>
                </Link>
              </div>
