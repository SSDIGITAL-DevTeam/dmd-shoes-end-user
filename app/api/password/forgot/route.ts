...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const upstream = await fetch(`${API_BASE}/password/forgot${search}`, {
      method: "POST",
      headers: {
    ...
    if (!ok) {
      const payload = data ?? { message: text || "Password reset email failed." };
      return NextResponse.json(payload, { status });
    }

    return NextResponse.json(data ?? { status: "ok" }, { status });
  } catch (error) {
    console.error("Proxy /password/forgot failed:", error);
    return NextResponse.json(
