...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const upstream = await fetch(`${API_BASE}/email/resend${search}`, {
      method: "POST",
      headers: {
    ...
    if (!ok) {
      const payload = data ?? { message: text || "Resend verification failed." };
      return NextResponse.json(payload, { status });
    }

    return NextResponse.json(data ?? { status: "ok" }, { status });
  } catch (error) {
    console.error("Proxy /email/resend failed:", error);
    return NextResponse.json(
